...
class Monitor(gym.Wrapper):
    ...
    def reset(self, **kwargs) -> GymObs:
        ...
        self.rewards = [[] for i in range(len(ret))]
        return ret
    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        for r in range(len(reward)):
            self.rewards[r].append(reward[r])
        if all(done):
            self.needs_reset = True
            arrived = sum([info_n['arrived'] for info_n in info])
            ep_rew = self.mean([sum(self.rewards[r]) for r in range(len(self.rewards))])
            ep_len = len(self.rewards[0])
            ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6), "arrived":arrived}
